...
| LED blink pattern |
|<center>⚠️<br><br>1 very long blink, lasting 5 seconds | <li>Use a supported partition (FAT32, exFAT, NTFS, ext4)<>Repair partition table<>Try another USB drive<>Try another USB OTG adapter |
|<center>⚠️<br>nbspnbsp<br> long blinks, each lasting 1.5 seconds | femtofox-config.txt is configured to skip if log exists. | USB drive mounted successfully but femtofox-config.txt contains `dont_run_if_log_exists="true"` and a log is present on the USB drive. Ignoring. | Remove log from USB drive or remove `dont_run_if_log_exists` line from femtofox-config.txt |
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyNDQxMzIzXX0=
-->